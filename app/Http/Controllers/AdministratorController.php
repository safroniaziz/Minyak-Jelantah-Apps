...
class AdministratorController extends Controller
{
    public function index(){
        $administrators = User::where('role','administrator')->orderBy('created_at','desc')->get();
    ...
    public function store(Request $request){
        ...
        if ($simpan) {
            return response()->json([
                'text'  =>  'Berhasil, data administrator berhasil disimpan',
                'url'   =>  route('administrator'),
            ]);
        }else {
            return response()->json(['text' =>  'Gagal, data administrator gagal disimpan']);
        }
    }
    ...
    public function update(Request $request, User $administrator){
        ...
        $update = User::where('id',$administrator->id)->update([
            'nama_lengkap'                                   =>  $request->nama_lengkap,
            'email'                                   =>  $request->email,
            'nomor_whatsapp'                                       =>  $request->nomor_whatsapp,
            'alamat'                                  =>  $request->alamat,
        ...
        if ($update) {
            return response()->json([
                'text'  =>  'Berhasil, data administrator berhasil diubah',
                'url'   =>  route('administrator'),
            ]);
        }else {
            return response()->json(['text' =>  'Gagal, data administrator gagal diubah']);
        }
    }
    ...
    public function delete(User $administrator){
        ...
        if ($delete) {
            return response()->json([
                'text'  =>  'Berhasil, data administrator berhasil dihapus',
                'url'   =>  route('administrator'),
            ]);
        }else {
            return response()->json(['text' =>  'Gagal, data administrator gagal dihapus']);
        }
    }
